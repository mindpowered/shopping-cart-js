...
$hx_exports["shoppingcart"] = $hx_exports["shoppingcart"] || {};
...
$hxClasses["shoppingcart.CartModel"] = shoppingcart_CartModel;
shoppingcart_CartModel.__name__ = "shoppingcart.CartModel";
Object.assign(shoppingcart_CartModel.prototype, {
	__class__: shoppingcart_CartModel
	,id: null
	,storeId: null
});
class shoppingcart_CartRepository {
	...
	Add(cart,callback) {
		let cartId = Std.string(this.counter);
		this.counter += 1;
		cart.id = cartId;
	...
	Remove(cartId,callback) {
		...
		while(_g < _g1.length) {
			...
			++_g;
			if(cart.id == cartId) {
				HxOverrides.remove(this.carts,cart);
				callback();
				return;
			}
		}
...
$hxClasses["shoppingcart.CartRepository"] = shoppingcart_CartRepository;
shoppingcart_CartRepository.__name__ = "shoppingcart.CartRepository";
Object.assign(shoppingcart_CartRepository.prototype, {
	__class__: shoppingcart_CartRepository
	,bus: null
	,carts: null
	,counter: null
});
...
$hxClasses["shoppingcart.ItemModel"] = shoppingcart_ItemModel;
shoppingcart_ItemModel.__name__ = "shoppingcart.ItemModel";
Object.assign(shoppingcart_ItemModel.prototype, {
	__class__: shoppingcart_ItemModel
	,cartId: null
	,idx: null
	,itemId: null
	,qty: null
...
class shoppingcart_ItemRepository {
	...
	Add(item,callback) {
		if(!Object.prototype.hasOwnProperty.call(this.items.h,item.cartId)) {
			let this1 = this.items;
			let k = item.cartId;
			let v = [];
			this1.h[k] = v;
		}
		let idx = this.items.h[item.cartId].length;
		item.idx = idx;
		this.items.h[item.cartId].push(item);
		callback(idx);
	}
	Edit(item,callback) {
		this.items.h[item.cartId][item.idx] = item;
		callback();
	}
	Remove(cartId,idx,callback) {
		this.items.h[cartId][idx] = null;
		callback();
	}
	FindByCartId(cartId,callback) {
		if(Object.prototype.hasOwnProperty.call(this.items.h,cartId)) {
			callback(this.items.h[cartId]);
		} else {
			callback([]);
		}
	}
	FindByCartIdAndIdx(cartId,idx,callback) {
		if(Object.prototype.hasOwnProperty.call(this.items.h,cartId)) {
			if(this.items.h[cartId].length > idx) {
				callback(this.items.h[cartId][idx]);
				return;
			}
		}
		callback(null);
...
$hxClasses["shoppingcart.ItemRepository"] = shoppingcart_ItemRepository;
shoppingcart_ItemRepository.__name__ = "shoppingcart.ItemRepository";
Object.assign(shoppingcart_ItemRepository.prototype, {
	__class__: shoppingcart_ItemRepository
	,bus: null
	,items: null
});
class shoppingcart_ShoppingCart {
	constructor(maglev) {
		this.maglev = maglev;
		this.logic = new shoppingcart_ShoppingCartLogic(new shoppingcart_CartRepository(maglev),new shoppingcart_ItemRepository(maglev));
		this.registerMyMethods();
	}
	registerMyMethods() {
		let _gthis = this;
		this.maglev.register("ShoppingCart.Create",maglev_MagLevFunction.fromFunction(function(args) {
			let storeId = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let result = maglev_MagLevResult.createAsync();
			_gthis.logic.Create(storeId,function(cartId) {
				result.setResult(maglev_MagLevString.fromString(cartId));
			});
			return result;
		}));
		this.maglev.register("ShoppingCart.AddItem",maglev_MagLevFunction.fromFunction(function(args) {
			let cartId = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let itemId = (js_Boot.__cast(args.get(1) , maglev_MagLevString)).getString();
			let qty = (js_Boot.__cast(args.get(2) , maglev_MagLevNumber)).getInt();
			let price = (js_Boot.__cast(args.get(3) , maglev_MagLevNumber)).getFloat();
			let result = maglev_MagLevResult.createAsync();
			_gthis.logic.AddItem(cartId,itemId,qty,price,function(idx) {
				result.setResult(maglev_MagLevNumber.fromInt(idx));
			});
			return result;
		}));
		this.maglev.register("ShoppingCart.ListItems",maglev_MagLevFunction.fromFunction(function(args) {
			let cartId = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let result = maglev_MagLevResult.createAsync();
			_gthis.logic.ListItems(cartId,function(items) {
		...
		this.maglev.register("ShoppingCart.RemoveItem",maglev_MagLevFunction.fromFunction(function(args) {
			let cartId = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let idx = (js_Boot.__cast(args.get(1) , maglev_MagLevNumber)).getInt();
			let result = maglev_MagLevResult.createAsync();
			_gthis.logic.RemoveItem(cartId,idx,function() {
				result.setResult(maglev_MagLevNull.create());
			});
			return result;
		}));
		this.maglev.register("ShoppingCart.UpdateQty",maglev_MagLevFunction.fromFunction(function(args) {
			let cartId = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let idx = (js_Boot.__cast(args.get(1) , maglev_MagLevNumber)).getInt();
			let qty = (js_Boot.__cast(args.get(2) , maglev_MagLevNumber)).getInt();
			let result = maglev_MagLevResult.createAsync();
			_gthis.logic.UpdateQty(cartId,idx,qty,function() {
				result.setResult(maglev_MagLevNull.create());
			});
			return result;
		}));
		this.maglev.register("ShoppingCart.CountItems",maglev_MagLevFunction.fromFunction(function(args) {
			let cartId = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let result = maglev_MagLevResult.createAsync();
			_gthis.logic.CountItems(cartId,function(count) {
				result.setResult(maglev_MagLevNull.create());
			});
			return result;
		}));
		this.maglev.register("ShoppingCart.GetCartSummary",maglev_MagLevFunction.fromFunction(function(args) {
			let cartId = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let result = maglev_MagLevResult.createAsync();
			_gthis.logic.GetCartSummary(cartId,function(summary) {
				let obj = maglev_MagLevObject.create();
		...
		this.maglev.register("ShoppingCart.Clear",maglev_MagLevFunction.fromFunction(function(args) {
			let result = maglev_MagLevResult.createAsync();
			let cartId = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			_gthis.logic.Clear(cartId,function() {
				result.setResult(maglev_MagLevNull.create());
			});
			return result;
		}));
...
$hxClasses["shoppingcart.ShoppingCart"] = $hx_exports["shoppingcart"]["ShoppingCart"] = shoppingcart_ShoppingCart;
shoppingcart_ShoppingCart.__name__ = "shoppingcart.ShoppingCart";
Object.assign(shoppingcart_ShoppingCart.prototype, {
	__class__: shoppingcart_ShoppingCart
	,maglev: null
	,logic: null
});
class shoppingcart_ShoppingCartLogic {
	...
	Create(storeId,callback) {
		let cart = new shoppingcart_CartModel();
		cart.storeId = storeId;
		this.cartRepo.Add(cart,function(cartId) {
			callback(cartId);
		});
	}
	AddItem(cartId,itemId,qty,price,callback) {
		let item = new shoppingcart_ItemModel();
		item.cartId = cartId;
		item.itemId = itemId;
		item.qty = qty;
		item.price = price;
		this.itemRepo.Add(item,function(idx) {
			callback(idx);
		});
	}
	ListItems(cartId,callback) {
		this.itemRepo.FindByCartId(cartId,function(items) {
			let list = [];
			let _g = 0;
			while(_g < items.length) {
				let item = items[_g];
				++_g;
				let cartItem = { idx : item.idx, itemId : item.itemId, qty : item.qty, price : item.price};
				list.push(cartItem);
			}
			callback(list);
		});
	}
	RemoveItem(cartId,idx,callback) {
		this.itemRepo.Remove(cartId,idx,function() {
			callback();
		});
	}
	UpdateQty(cartId,idx,qty,callback) {
		let _gthis = this;
		this.itemRepo.FindByCartIdAndIdx(cartId,idx,function(item) {
			item.qty = qty;
			_gthis.itemRepo.Edit(item,function() {
				callback();
			});
		});
	}
	CountItems(cartId,callback) {
		this.itemRepo.FindByCartId(cartId,function(items) {
			let count = items.length;
			callback(count);
		});
	}
	GetCartSummary(cartId,callback) {
		this.itemRepo.FindByCartId(cartId,function(items) {
			let total = 0;
			let count = 0;
			let _g = 0;
			...
			let summary = { total : total, items : count};
			callback(summary);
		});
	}
	Clear(cartId,callback) {
		this.cartRepo.Remove(cartId,function() {
			callback();
		});
	}
}
$hxClasses["shoppingcart.ShoppingCartLogic"] = shoppingcart_ShoppingCartLogic;
shoppingcart_ShoppingCartLogic.__name__ = "shoppingcart.ShoppingCartLogic";
Object.assign(shoppingcart_ShoppingCartLogic.prototype, {
	__class__: shoppingcart_ShoppingCartLogic
	,cartRepo: null
	,itemRepo: null
});
...
shoppingcart_ShoppingCart.__meta__ = { obj : { SuppressWarnings : [["checkstyle:FieldDocComment","checkstyle:LocalVariableName","checkstyle:MultipleStringLiterals","checkstyle:MagicNumber"]]}};
})(typeof window != "undefined" ? window : typeof global != "undefined" ? global : typeof self != "undefined" ? self : this);
var shoppingcart = $hx_exports["shoppingcart"];
